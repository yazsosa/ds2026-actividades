...
export default function App() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      ...
      <main style={{ flex: 1, maxWidth: '1100px', margin: '3rem auto', padding: '0 1.5rem', width: '100%' }}>
        <h2 style={{ fontSize: '1.6rem', fontWeight: 700, marginBottom: '2rem', color: '#2b3a2b' }}>
          {query ? `Resultados para "${query}"` : 'Libros destacados'}
        </h2>
        {filtered.length === 0 ? (
          <p style={{ color: '#5a7a7a' }}>No se encontraron libros.</p>
        ) : (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1.5rem' }}>
            {filtered.map(book => <BookCard key={book.id} {...book} />)}
          </div>
        )}
      </main>
      <Footer />
